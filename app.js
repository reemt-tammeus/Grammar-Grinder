...
// --- NEU: Touch & Klick kombiniert für sofortige Reaktion auf Handys ---
function bindTouchButton(id, action) {
    const btn = document.getElementById(id);
    if (!btn) return;
    btn.onpointerdown = (e) => {
        e.preventDefault(); // Verhindert doppeltes Auslösen durch Ghost-Clicks
        action();
    };
}

window.onload = () => {
    bindTouchButton('btn-ap', () => startApp('ap'));
    bindTouchButton('btn-quickie', () => startApp('quickie'));
    bindTouchButton('btn-goto-menu', () => location.reload());
};
...
async function startApp(mode) {
    ...
    try {
        ...
        alert(`FEHLER BEIM STARTEN:\n${e.message}\n\nDie App wurde angehalten. Drücke F12 für die Konsole.`);
...
function renderKeyboard() {
    ...
    keys.forEach(row => {
        ...
        row.forEach(k => {
            ...
            // --- NEU: Touch & Klick für die Tastatur kombiniert ---
            b.onpointerdown = (e) => {
